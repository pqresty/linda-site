...
def times(events):
    ...
    # Сбор площадки называют по-разному: «сбор гостей», «открытие дверей»,
    # просто «двери». Пока в шаблоне стоял один «сбор», Аврора пролетала мимо
    # разбора целиком — у неё написано «Открытие дверей - 19:00 Начало
    # концерта - 20:00», и не совпадало ни первое слово, ни тире вместо
    # двоеточия. Такие площадки молча уезжали в «не удалось сверить», а он из
    # недельной сводки вырезан — то есть не замечались вовсе.
    NACH = re.compile(
        r'(?:(?:[Сс]бор|[Оо]ткрыти\w* двер\w*|[Дд]вери|[Вв]ход)'
        r'[^.]{0,40}?(\d{1,2}:\d{2})[^.]{0,20}?)?'
        r'[Нн]ачал\w*\s+(?:выступлени\w*|концерта|программы|шоу|в)'
        r'[\s:.\u2014\u2013-]*(?:в\s*)?(\d{1,2}:\d{2})')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
        for e, why, found in ex.map(look, live):
            ...
            notes = []
            if start != e["time"]:
                notes.append(f'начало {e["time"]} → {start}')
            # Сбор сверяем отдельно: площадка может объявить его позже, чем
            # завела страницу, а показываем мы как раз раннее время — то есть
            # появившийся сбор меняет строку, хотя начало осталось прежним.
            if sbor and sbor != e.get("doors"):
                notes.append(f'сбор {e.get("doors") or "не был указан"} → {sbor}')
            if notes:
                shown = min(x for x in (sbor or e.get("doors"), start) if x)
                diff.append((e, notes, shown))

    if diff:
        print("время разошлось с площадкой:")
        for e, notes, shown in sorted(diff, key=lambda x: x[0]["date"]):
            print(f'  {e["date"]} {e["city"]} · {e.get("venue","")}: '
                  f'{"; ".join(notes)} — в строке будет {shown}')
    else:
        print("время везде совпадает")
    if unknown:
